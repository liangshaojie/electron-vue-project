<template>
  <div class="slider-container" :style="sliderStyle" @mouseover="pause()" @mouseout="play()" v-if="list.length">
    <div ref="content" class="slider-content" :class="mask ? 'mask' : ''">
      <div ref="slider" class="slider" v-for="(item, index) in list" :key="index" :class="setClass(index)" :style="`backgroundImage: url(${item.src});height: ${height}px;`">
